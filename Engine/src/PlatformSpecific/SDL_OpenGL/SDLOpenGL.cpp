...
SDL_OpenGL::SDL_OpenGL(const ApplicationSpecification& applicationSpecification /*= ApplicationSpecification()*/)
...
void SDL_OpenGL::SetMenubarCallback(const std::function<void() >& menubarCallback)
{
	//throw std::logic_error("The method or operation is not implemented.");
}

void SDL_OpenGL::Close()
{
	m_Running = false;
}

void SDL_OpenGL::PushLayer(const std::shared_ptr<Layer>& layer)
{
	//throw std::logic_error("The method or operation is not implemented.");
}

void SDL_OpenGL::Init()
{
	// Setup SDL
	...
	static SDL_WindowFlags window_flags = (SDL_WindowFlags)(SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE | SDL_WINDOW_ALLOW_HIGHDPI);
	...
	if(!ImGui_ImplSDL2_InitForOpenGL(m_WindowHandle, m_Context))
	{
	...
	{
		printf("Failed to initialize OpenGL3\n");
	}
}

void SDL_OpenGL::Shutdown()
{
	for (auto& layer : m_LayerStack)
		layer->OnDetach();
